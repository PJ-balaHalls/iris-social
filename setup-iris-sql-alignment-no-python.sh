...
function patchFile(path, patches) {
  if (!fs.existsSync(path)) {
    console.log(`ℹ️ ${path} não encontrado, pulando.`);
    return;
  }
  ...
  if (changed) {
    ...
    console.log(`ℹ️ ${path} já estava atualizado ou bloco não encontrado.`);
  }
}

patchFile('src/components/iris/IrisProfileProvider.tsx', [
  [
...
patchFile('src/lib/auth/routes.ts', [
  [
...
patchFile('src/lib/auth/resolvePostAuthDestination.ts', selectPatches);
patchFile('middleware.ts', selectPatches);
patchFile('src/app/api/auth/post-auth-destination/route.ts', selectPatches);
NODE
...
const path = 'src/components/settings/SettingsIcon.tsx';

if (!fs.existsSync(path)) {
  console.log('ℹ️ SettingsIcon.tsx não encontrado, pulando.');
  process.exit(0);
}

let text = fs.readFileSync(path, 'utf8');

if (!text.includes('CreditCard: Icons.CreditCard,')) {
  text = text.replace(
    'Crown: Icons.Crown,',
...
if (!text.includes('Leaf: Icons.Leaf,')) {
  text = text.replace(
    'Image: Icons.Image,',
    'Image: Icons.Image,\n  Leaf: Icons.Leaf,'
  );
...
fs.writeFileSync(path, text);
console.log('✅ SettingsIcon atualizado.');
NODE
...
if (!fs.existsSync(path)) {
  console.log('⚠️ src/app/(app)/settings/page.tsx não encontrado. Rode o setup da settings primeiro.');
  process.exit(0);
}

let text = fs.readFileSync(path, 'utf8');

if (!text.includes('const [subscription, setSubscription]')) {
  text = text.replace(
...
const oldBlock = `    setProfile(data);
    setLoading(false);
    setSyncMessage('Perfil sincronizado');`;

const newBlock = `    const { data: subscriptionData } = await supabase
      .from('subscriptions')
      .select('*')
      .eq('user_id', user.id)
      .maybeSingle();
...
if (text.includes(oldBlock) && !text.includes("from('subscriptions')")) {
  text = text.replace(oldBlock, newBlock);
}

const oldPanel = `          <SettingsDetailPanel
            option={selectedOption}
            profile={profile}
          />`;

const newPanel = `          <SettingsDetailPanel
            option={selectedOption}
            profile={profile}
...
if (text.includes(oldPanel)) {
  text = text.replace(oldPanel, newPanel);
}

fs.writeFileSync(path, text);
console.log('✅ settings/page.tsx alinhado ao SQL atualizado.');
NODE

echo "✅ FE-IRIS-052 aplicado sem Python: alinhamento com SQL atualizado, full_name, subscriptions e settings."
